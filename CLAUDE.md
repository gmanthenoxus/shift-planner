...
HTML/CSS/vanilla JS, single file `index.html`, no framework, no build step. (v11's three-file exception for a service worker lapsed with 2.0: no PWA in scope, so no service worker, so no exception.) · localStorage-first persistence (key `"shiftPlanner.v5"`, versioned independently of feature version) · GitHub Pages deployment, legacy branch-deploy from `main:/` (correct here, there is nothing to build) · icons: none currently in use, if icons are added, the design system's global icon rule applies (one proper library, no emoji) · Interaction Tier: Tier 1 (DOM/CSS only) throughout; no Tier 2/3 need identified.
...
- **Freeze:** no code before SCOPE.md is committed with a freeze line. (2.0 frozen 2026-08-19.)
