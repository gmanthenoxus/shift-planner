...
<!-- To freeze: read it, cut what you don't want, answer the six defaults in §7, copy the whole
     ...
     (v10's is still missing), commit. No code before that. Checkpoint 1, never routed around.

     Inputs merged: SCOPE.md v10 (frozen 18 Aug) · SPEC-v11-TARGET.html (flow reference only,
     its Engineer tokens are superseded) · PROMPTS.md §2 (D1-D5) · REQUIREMENTS.md (legal
     trigger map) · decisions D6-D10 (19 Aug) · DESIGN-TOKENS.md · COPY-DECK.md. -->
...
<!-- The rest of the compliance surface (privacy notice, ICO registration, s.164A complaints
     route) is triggered by COLLECTING personal data. v11 collects nothing, so those defer with
     the feedback route to v12. These three do not depend on collection and ship now. -->
